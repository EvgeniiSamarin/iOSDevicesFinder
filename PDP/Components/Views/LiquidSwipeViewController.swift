...
open class LiquidSwipeContainerController: UIViewController {
    ...
    public var liquidDelegate: LiquidSwipeContainerDelegate?
    ...
    @objc private func trailingEdgePan(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .began {
            ...
            if let viewController = nextViewController {
                liquidDelegate?.liquidSwipeContainer(self, willTransitionTo: viewController)
            }
                ...
                guard let gesture = sender,
                      ...
                      let time = animation?.elapsedTime else {
                        if let viewController = self.nextViewController {
                            self.liquidDelegate?.liquidSwipeContainer(self, didFinishTransitionTo: viewController, transitionCompleted: false)
                        }
                    return false
                }
                ...
                guard let gesture = sender,
                    ...
                    let time = animation?.elapsedTime else {
                        if let nextViewController = self.nextViewController {
                            self.liquidDelegate?.liquidSwipeContainer(self, didFinishTransitionTo: nextViewController, transitionCompleted: false)
                        }
                        return false
                }
                let speed: CGFloat = 2000
    ...
    private func showNextPage() {
        ...
        guard nextViewController != nil else {
            ...
            if let viewController = currentViewController {
                liquidDelegate?.liquidSwipeContainer(self, didFinishTransitionTo: viewController, transitionCompleted: true)
            }
            return
        }
        ...
        animation?.completionBlock = { (_,_) in
            if let viewController = self.currentViewController {
                self.liquidDelegate?.liquidSwipeContainer(self, didFinishTransitionTo: viewController, transitionCompleted: true)
            }
        }
        currentPage?.pop_add(animation, forKey: "animation")
    }
        ...
        coordinator.animate(alongsideTransition: { (_) in
            ...
            if let currentPage = self.currentPage {
                ...
                let maskLayer = WaveLayer(waveCenterY: self.initialWaveCenter,
                                          ...
                                          sideWidth: hasNextPage ?self.initialSideWidth : 0,
                                          isRTL: self.view.isRTL)
